...
function saveAppState() {
  ...
  /* Single point where the app's "last updated" timestamp is captured — generated
     once per actual state change (not on every re-render), reused everywhere via
     formatDateTime(appState.lastUpdatedAt) and persisted so it survives refresh. */
...
function renderNotifications() {
  ...
  list.innerHTML = appState.notifications.length ? appState.notifications.map(n => `
    <div class="notification-item ${n.read ? 'read' : 'unread'}" data-notification-id="${n.id}">
      ...
      <div class="notification-item-meta">${formatDateTime(n.date)}</div>
    </div>
...
function buildApplicationDetailsHtml(app) {
  ...
  return `
    <div style="display:grid;gap:12px;">
      ...
      <p><strong>Submission Time:</strong> ${escapeHtml(formatDateTime(normalized.createdAt || normalized.dateTime || normalized.date))}</p>
...
function createApplicationRecord(job, status, extra = {}) {
  ...
  /* Single timestamp captured once, at the moment this application record is
     created, and reused everywhere for this job (createdAt/date/time all
     derive from the same `now`). It is stored in the record (and therefore
     in localStorage via saveAppState), so every subsequent normalization or
     re-render reuses this exact value instead of generating a new one. */
  const now = new Date();
  ...
  const application = normalizeApplicationRecord({
    ...
    applicationUrl,
    applyUrl,
    matchScore: Number.isFinite(Number(job.matchScore)) ? Number(job.matchScore) : 0,
...
function persistApplication(job, status, extra = {}) {
  ...
  const normalized = normalizeApplicationRecord({
    ...
    applicationUrl,
    applyUrl,
    matchScore: Number.isFinite(Number(job.matchScore)) ? Number(job.matchScore) : (Number.isFinite(Number(application.matchScore)) ? Number(application.matchScore) : 0),
...
/* ---------------------------------------------------------------------
   formatDateTime() — THE single shared helper used everywhere a full,
   friendly date + time needs to be displayed (Profile Last Updated,
   Resume Uploaded date, Dashboard "Updated" time, Applications table
   Date column, Application Details "Submission Time", Recent Activity,
   Workflow Completed summary, and Notifications). There is intentionally
   only one such formatting function in the whole project — every call
   site below reuses it instead of re-implementing its own date/time
   formatting, so the display format can never drift between pages.
   --------------------------------------------------------------------- */
...
function isMatchedDashboardJob(job) {
  const normalized = normalizeApplicationRecord(job, job);
  const threshold = Number(profileState.minMatchScore || 75);
  return Boolean(normalized.matched || Number(normalized.matchScore) >= threshold || Number(normalized.matchScore) > 0);
}
...
function getDashboardRecords(category) {
  ...
  switch (category) {
    case 'jobsFound':
      return jobs;
    case 'jobsMatched':
      return jobs.filter(job => isMatchedDashboardJob(job));
    case 'applicationsSent':
      return applications.filter(app => isDashboardSubmissionAttempt(app));
    case 'successfulApplications':
      return applications.filter(app => app.status === 'Success');
    case 'pendingReview':
      return applications.filter(app => ['Pending Review', 'Manual Action Needed'].includes(app.status));
    case 'temporaryFailure':
      return applications.filter(app => app.status === 'Temporary Failure');
    case 'permanentFailure':
      return applications.filter(app => app.status === 'Permanent Failure');
    case 'skipped':
      return applications.filter(app => app.status === 'Skipped');
    default:
      return [];
  }
}

function getDashboardCategoryMeta(category) {
  switch (category) {
    case 'jobsFound': return { label: 'Jobs Found', empty: 'No jobs found yet.' };
    case 'jobsMatched': return { label: 'Jobs Matched', empty: 'No matched jobs available.' };
    case 'applicationsSent': return { label: 'Applications Sent', empty: 'No submission attempts yet.' };
    case 'successfulApplications': return { label: 'Successful Applications', empty: 'No successful applications yet.' };
    case 'pendingReview': return { label: 'Pending Review / Manual Action Needed', empty: 'No pending review items.' };
    case 'temporaryFailure': return { label: 'Temporary Failure', empty: 'No temporary failures.' };
    case 'permanentFailure': return { label: 'Permanent Failure', empty: 'No permanent failures.' };
    case 'skipped': return { label: 'Skipped', empty: 'No skipped jobs.' };
    default: return { label: 'Dashboard Details', empty: 'No records available.' };
  }
}

function getSafeText(value, fallback = 'Not provided') {
...
function formatSafeDate(value) {
  ...
  if (!(parsed instanceof Date) || Number.isNaN(parsed.getTime())) {
    const fallback = getSafeText(value, 'Not provided');
    return fallback === 'Not provided' ? 'Not provided' : fallback;
  }
  return formatDateTime(parsed.toISOString());
}

function getDashboardRecordDetails(record) {
  const normalized = normalizeApplicationRecord(record, record);
  const skills = Array.isArray(normalized.skills) ? normalized.skills.filter(Boolean) : [];
  const skillsText = skills.length ? skills.join(', ') : getSafeText(normalized.requiredSkills || normalized.preferredSkills || normalized.skillsText || '', 'Not provided');
  const description = getSafeText(normalized.description || normalized.jobDescription || normalized.summary || '', 'No description provided.');
  const salary = getSafeText(normalized.salary || normalized.salaryRange || normalized.compensation || normalized.pay || '', 'Salary not listed');
  const jobType = getSafeText(normalized.employmentType || normalized.jobType || normalized.positionType || '', 'Not provided');
  const experience = getSafeText(normalized.experienceLevel || normalized.experience || '', 'Not provided');
  const matchScore = Number.isFinite(Number(normalized.matchScore)) && Number(normalized.matchScore) > 0 ? `${normalized.matchScore}%` : 'Not provided';
  const location = getSafeText(normalized.location || 'Location unavailable', 'Location unavailable');
  const company = getSafeText(normalized.company || 'Unknown Company', 'Unknown Company');
  const title = getSafeText(normalized.jobTitle || normalized.title || 'Job Opportunity', 'Job Opportunity');
  const source = getSafeText(normalized.source || 'Unknown Source', 'Unknown Source');
  const status = getSafeText(normalized.status || 'Pending Review', 'Pending Review');
  const dateValue = formatSafeDate(normalized.createdAt || normalized.dateTime || normalized.date || normalized.postedDate || normalized.applicationDate);
  const fallbackUrl = getSafeText(normalized.originalJobUrl || normalized.applicationUrl || normalized.applyUrl || '', 'Not provided');
  return {
    title,
    company,
    location,
    ...
    description,
    skillsText,
    matchingSkills: getSafeText(normalized.matchingSkills || normalized.matchedSkills || '', 'Not provided'),
    missingSkills: getSafeText(normalized.missingSkills || '', 'Not provided'),
    aiMatchReason: getSafeText(normalized.aiMatchReason || normalized.matchReason || '', 'Not provided'),
    originalJobUrl: fallbackUrl,
    failureReason: getSafeText(normalized.failureReason || '', 'Not provided'),
    manualActionReason: getSafeText(normalized.manualActionReason || '', 'Not provided')
  };
}

function filterAndSortDashboardRecords(records, category, query, sortBy) {
  const term = String(query || '').trim().toLowerCase();
  const filtered = (records || []).filter(record => {
    if (!term) return true;
    const details = getDashboardRecordDetails(record);
    const haystack = [details.title, details.company, details.location, details.skillsText, details.status].join(' ').toLowerCase();
    return haystack.includes(term);
  });
  return filtered.sort((a, b) => {
    const left = getDashboardRecordDetails(a);
    const right = getDashboardRecordDetails(b);
    switch (sortBy) {
      case 'oldest':
        return new Date(a.createdAt || a.dateTime || a.date || 0) - new Date(b.createdAt || b.dateTime || b.date || 0);
      case 'highestMatch':
      ...
      case 'newest':
      default:
        return new Date(b.createdAt || b.dateTime || b.date || 0) - new Date(a.createdAt || a.dateTime || a.date || 0);
    }
  });
}

let dashboardDrawerState = { isOpen: false, category: null, view: 'list', record: null, trigger: null, lastScrollY: 0 };

function openDashboardDrawer(category, triggerElement) {
  const drawer = document.getElementById('dashboardDetailsDrawer');
  const backdrop = document.getElementById('dashboardDrawerBackdrop');
  const titleEl = document.getElementById('dashboardDrawerTitle');
  const countEl = document.getElementById('dashboardDrawerCount');
  const closeBtn = document.getElementById('dashboardDrawerClose');
  if (!drawer || !backdrop || !titleEl || !countEl || !closeBtn) return;

  dashboardDrawerState.isOpen = true;
  dashboardDrawerState.category = category;
  dashboardDrawerState.view = 'list';
  dashboardDrawerState.record = null;
  dashboardDrawerState.trigger = triggerElement || null;
  dashboardDrawerState.lastScrollY = window.scrollY || document.documentElement.scrollTop || 0;

  const meta = getDashboardCategoryMeta(category);
  ...
  document.body.style.top = `-${dashboardDrawerState.lastScrollY}px`;
...
function closeDashboardDrawer() {
  ...
  window.scrollTo(0, dashboardDrawerState.lastScrollY || 0);
  if (dashboardDrawerState.trigger && typeof dashboardDrawerState.trigger.focus === 'function') {
    dashboardDrawerState.trigger.focus();
  }
  dashboardDrawerState.isOpen = false;
  dashboardDrawerState.category = null;
  dashboardDrawerState.view = 'list';
  dashboardDrawerState.record = null;
  dashboardDrawerState.trigger = null;
}

function renderDashboardDrawerContent() {
  const drawer = document.getElementById('dashboardDetailsDrawer');
  const contentEl = document.getElementById('dashboardDrawerContent');
  const titleEl = document.getElementById('dashboardDrawerTitle');
  const countEl = document.getElementById('dashboardDrawerCount');
  const searchEl = document.getElementById('dashboardDrawerSearch');
  const sortEl = document.getElementById('dashboardDrawerSort');
  const toolbarEl = document.querySelector('.dashboard-drawer-toolbar');
  if (!drawer || !contentEl || !titleEl || !countEl) return;

  const category = dashboardDrawerState.category;
  const meta = getDashboardCategoryMeta(category);
  const records = getDashboardRecords(category);
  const query = (searchEl && searchEl.value) || '';
  const sortBy = (sortEl && sortEl.value) || 'newest';
  const visibleRecords = filterAndSortDashboardRecords(records, category, query, sortBy);

  titleEl.textContent = meta.label;
  countEl.textContent = `${visibleRecords.length} ${visibleRecords.length === 1 ? 'record' : 'records'}`;
  if (toolbarEl) toolbarEl.classList.toggle('hidden', dashboardDrawerState.view === 'detail');

  if (dashboardDrawerState.view === 'detail' && dashboardDrawerState.record) {
    const details = getDashboardRecordDetails(dashboardDrawerState.record);
    contentEl.innerHTML = `
      <div class="dashboard-drawer-detail">
        <button type="button" class="dashboard-drawer-action-btn dashboard-drawer-back-btn" data-dashboard-action="back-to-list">← Back to List</button>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Job Title</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.title)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Company</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.company)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Location</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.location)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Salary</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.salary)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Employment Type</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.jobType)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Experience Level</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.experience)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Full Job Description</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.description)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Required Skills</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.skillsText)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Match Score</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.matchScore)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Matching Skills</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.matchingSkills)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Missing Skills</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.missingSkills)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">AI Match Reason</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.aiMatchReason)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Source</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.source)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Original Job URL</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.originalJobUrl)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Date Found</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.dateValue)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Current Status</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.status)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Failure Reason</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.failureReason)}</div>
        </div>
        <div class="dashboard-drawer-detail-card">
          <div class="dashboard-drawer-detail-label">Manual Action Reason</div>
          <div class="dashboard-drawer-detail-value">${escapeHtml(details.manualActionReason)}</div>
        </div>
      </div>
    `;
    return;
  ...
  if (!visibleRecords.length) {
    contentEl.innerHTML = `<div class="dashboard-drawer-empty">${escapeHtml(meta.empty)}</div>`;
    return;
  }

  contentEl.innerHTML = `
    <div class="dashboard-drawer-list">
      ${visibleRecords.map(record => {
        const details = getDashboardRecordDetails(record);
        const existingApp = findExistingApplicationForJob(record);
        const buttons = [];
        buttons.push(`<button type="button" class="dashboard-drawer-action-btn" data-dashboard-action="view-details" data-dashboard-record-id="${record.id || ''}">View Details</button>`);
        if (details.status === 'Temporary Failure') {
          const appRecord = existingApp || sampleApplications.find(app => app.id === record.id || app.jobKey === record.jobKey);
          const retryDisabled = Boolean(appRecord && (appRecord.retryCount || 0) >= MAX_RETRY_COUNT);
          if (!retryDisabled) buttons.push(`<button type="button" class="dashboard-drawer-action-btn" data-dashboard-action="retry" data-dashboard-record-id="${record.id || ''}">Retry</button>`);
        }
        if (['Pending Review', 'Manual Action Needed'].includes(details.status)) {
          buttons.push(`<button type="button" class="dashboard-drawer-action-btn" data-dashboard-action="complete-application" data-dashboard-record-id="${record.id || ''}">Complete Application</button>`);
        }
        if (category === 'jobsFound' || category === 'jobsMatched') {
          if (!existingApp) buttons.push(`<button type="button" class="dashboard-drawer-action-btn" data-dashboard-action="apply" data-dashboard-record-id="${record.id || ''}">Apply</button>`);
        }
        return `
          <div class="dashboard-drawer-item">
            <div class="dashboard-drawer-item-title">${escapeHtml(details.title)}</div>
            <div class="dashboard-drawer-item-meta">
              <div><strong>Company:</strong> ${escapeHtml(details.company)}</div>
              <div><strong>Location:</strong> ${escapeHtml(details.location)}</div>
              <div><strong>Salary:</strong> ${escapeHtml(details.salary)}</div>
              <div><strong>Type:</strong> ${escapeHtml(details.jobType)}</div>
              <div><strong>Experience:</strong> ${escapeHtml(details.experience)}</div>
              <div><strong>Match:</strong> ${escapeHtml(details.matchScore)}</div>
              <div><strong>Status:</strong> ${escapeHtml(details.status)}</div>
              <div><strong>Source:</strong> ${escapeHtml(details.source)}</div>
              <div><strong>Date:</strong> ${escapeHtml(details.dateValue)}</div>
              <div><strong>Description:</strong> ${escapeHtml(details.description)}</div>
              <div><strong>Skills:</strong> ${escapeHtml(details.skillsText)}</div>
            </div>
            <div class="dashboard-drawer-item-footer">${buttons.join('')}</div>
          </div>
        `;
      }).join('')}
    </div>
...
function renderDashboard() {
  ...
  const cards = [
    { label: 'Jobs Found', value: stats.jobsFound, icon: '📄', color: '#334155', category: 'jobsFound' },
    { label: 'Jobs Matched', value: stats.jobsMatched, icon: '🎯', color: '#7c3aed', category: 'jobsMatched' },
    { label: 'Applications Sent', value: stats.applicationsSent, icon: '📤', color: '#2563eb', category: 'applicationsSent' },
    { label: 'Successful Applications', value: stats.successful, icon: '✅', color: '#16a34a', category: 'successfulApplications' },
    { label: 'Pending Review / Manual Action Needed', value: stats.pendingReviews, icon: '⏳', color: '#ea580c', category: 'pendingReview' },
    { label: 'Temporary Failure', value: stats.temporaryFailures, icon: '⏱', color: '#f59e0b', category: 'temporaryFailure' },
    { label: 'Permanent Failure', value: stats.permanentFailures, icon: '❌', color: '#dc2626', category: 'permanentFailure' },
    { label: 'Skipped', value: stats.skipped, icon: '⏭', color: '#64748b', category: 'skipped' },
  ];
  grid.innerHTML = cards.map(c => `
    <div class="dash-card dash-card-clickable" role="button" tabindex="0" data-dashboard-category="${c.category}" aria-label="Open ${escapeHtml(c.label)} details">
    ...
    activityList.innerHTML = recent.length ? recent.map(a => {
      ...
      return `<div class="activity-item">
        ...
        <div class="activity-body">
          ...
          <div class="activity-meta">${escapeHtml(a.status || 'Pending Review')} • ${escapeHtml(formatDateTime(a.createdAt || a.dateTime || a.date))}</div>
        </div>
      </div>`;
  ...
  if (updatedEl) {
    /* Reuses the single lastUpdatedAt timestamp captured in saveAppState()
       whenever the app's data actually changed — does NOT generate a new
       timestamp on every render, so re-opening the Dashboard shows the
       same "Updated" time until a new job/action actually changes data. */
    updatedEl.textContent = 'Updated ' + formatDateTime(appState.lastUpdatedAt || new Date().toISOString());
  }
}
...
function renderApplicationsTable() {
  ...
  tbody.innerHTML = filtered.map(a => {
    ...
    return `<tr>
      ...
      <td data-label="Date">${escapeHtml(formatDateTime(a.createdAt || a.dateTime || a.date))}</td>
...
const dashStatsGridEl = document.getElementById('dashStatsGrid');
if (dashStatsGridEl) {
  dashStatsGridEl.addEventListener('click', (event) => {
    const card = event.target.closest('[data-dashboard-category]');
    if (!card) return;
    openDashboardDrawer(card.dataset.dashboardCategory, card);
  });
  dashStatsGridEl.addEventListener('keydown', (event) => {
    const card = event.target.closest('[data-dashboard-category]');
    if (!card) return;
...
const dashboardDrawerBackdropEl = document.getElementById('dashboardDrawerBackdrop');
if (dashboardDrawerBackdropEl) dashboardDrawerBackdropEl.addEventListener('click', closeDashboardDrawer);
const dashboardDrawerCloseEl = document.getElementById('dashboardDrawerClose');
if (dashboardDrawerCloseEl) dashboardDrawerCloseEl.addEventListener('click', closeDashboardDrawer);
const dashboardDrawerSearchEl = document.getElementById('dashboardDrawerSearch');
...
if (dashboardDrawerContentEl) {
  dashboardDrawerContentEl.addEventListener('click', async (event) => {
    ...
    if (!actionBtn) return;
    const action = actionBtn.dataset.dashboardAction;
    ...
    const recordId = actionBtn.dataset.dashboardRecordId;
    const category = dashboardDrawerState.category;
    const record = getDashboardRecords(category).find(item => String(item.id || '') === String(recordId));
    if (!record) return;
    if (action === 'view-details') {
      dashboardDrawerState.view = 'detail';
      dashboardDrawerState.record = record;
      renderDashboardDrawerContent();
      return;
    }
    if (action === 'retry') {
      const appRecord = sampleApplications.find(app => app.id === record.id || app.jobKey === record.jobKey);
      if (appRecord) await retryApplication(appRecord.id);
      return;
    }
    if (action === 'complete-application') {
      const appRecord = sampleApplications.find(app => app.id === record.id || app.jobKey === record.jobKey);
      if (appRecord) await handleMarkManualCompleted(appRecord.id);
      return;
    }
    if (action === 'apply') {
      persistApplication(record, 'Pending Review', { notes: 'Queued from dashboard drawer.', manualReviewRequired: true });
      showToast('Application queued for review.', 'info');
      return;
    }
  });
}
